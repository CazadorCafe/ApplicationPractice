...
interface DirectoryItemDao {
    @Query("SELECT id AS individualId, lastName, firstName, profilePicture FROM Individual ORDER BY firstName, lastName")
    fun findAllDirectItemsByFirstNameFlow(): Flow<List<DirectoryItemEntityView>>

    @Query("SELECT id AS individualId, lastName, firstName, profilePicture FROM Individual ORDER BY lastName, firstName")
